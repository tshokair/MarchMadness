{
  ...
  gROOT->SetStyle("Plain");
    ...
    for(int j=0;j<4;j++){
        ...
        for(int i=0;i<4;i++){
            rand= r.Rndm();
            if(probOfWinning[roundOneWinners[j][2*i]]>probOfWinning[roundOneWinners[j][2*i+1]]){
                temp=roundOneWinners[j][2*i+1];
                cerr<<"use probability for "<<probOfWinning[roundOneWinners[j][2*i+1]]<<endl;
            }
            else{
                temp=roundOneWinners[j][2*i];
                cerr<<"use probability for "<<roundOneWinners[j][2*i]<<endl;
            }
            if( rand >probOfWinning[temp] ){
            ...
            cerr<<"random number was "<<rand<<" temp was "<<temp<<endl;
    ...
    for(int j=0;j<4;j++){
        ...
        for(int i=0;i<2;i++){
            rand= r.Rndm();
            if(probOfWinning[roundTwoWinners[j][2*i]]>probOfWinning[roundTwoWinners[j][2*i+1]]){
                temp2=roundTwoWinners[j][2*i+1];
            }
            else{
                temp2=roundTwoWinners[j][2*i];
            }
            if( rand >probOfWinning[temp2] ){
    ...
    for(int j=0;j<4;j++){
        cerr<<"Part "<<j+1<<endl;
        if(probOfWinning[roundTwoWinners[j][0]]>probOfWinning[roundTwoWinners[j][1]]){
            temp3=roundTwoWinners[j][1];
        }
        else{
            temp3=roundTwoWinners[j][0];
        }
            rand= r.Rndm();
            if( rand <probOfWinning[temp3] ){
